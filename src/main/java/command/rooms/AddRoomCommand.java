...
public class AddRoomCommand implements Command {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            // Auto-increment logic if room number is empty
            if (roomNumber == null || roomNumber.trim().isEmpty()) {
                String latest = roomsService.getLatestRoomNumber();
                try {
                    int nextNum = Integer.parseInt(latest) + 1;
                    roomNumber = String.valueOf(nextNum);
                } catch (NumberFormatException e) {
                    // Fallback if latest is not a number
                    roomNumber = "101";
                }
            }
            ...
            Rooms newRoom = new Rooms(
                    ...
                    description,
                    status,
                    imageName.isEmpty() ? "" : "uploads/rooms/" + imageName
            );

            boolean success = roomsService.addRoom(newRoom);

            if (success) {
                response.sendRedirect(request.getContextPath() + "/rooms?action=list");
            } else {
                request.setAttribute("errorMessage", "Failed to add room.");
                request.getRequestDispatcher("/staff/addRoom.jsp").forward(request, response);
            }
        } catch (NumberFormatException e) {
            request.setAttribute("errorMessage", "Invalid numeric format for price or capacity.");
            request.getRequestDispatcher("/staff/addRoom.jsp").forward(request, response);
        } catch (Exception e) {
            e.printStackTrace();
