...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 relative text-white flex flex-col shadow-lg">
        ...
        <nav class="relative flex-1 px-4 py-6 space-y-2">
            <a href="<%= request.getContextPath() %>/staff/dashboard.jsp"
            ...
            <!-- Manage Event Packages Dropdown -->
            <div x-data="{ open: false }">
                <button @click="open = !open"
                    ...
                    <span class="flex items-center">
                        <i class="fas fa-gift mr-2 w-5"></i> Manage Events
                    </span>
                    <i class="fas fa-chevron-down text-xs transition-transform duration-200"
                        :class="{'rotate-180': open}"></i>
                </button>
                <div x-show="open" x-collapse class="mt-1 pl-8 space-y-1">
                    <a href="<%= request.getContextPath() %>/events?action=addForm"
                    ...
                    <a href="<%= request.getContextPath() %>/events?action=list"
                        class="block px-3 py-2 text-sm rounded-lg hover:bg-blue-600 transition text-blue-100 hover:text-white flex items-center">
                        <i class="fas fa-list mr-2 text-xs"></i> All Events
                    </a>
                </div>
            </div>

            <!-- Manage Food Packages Dropdown -->
            <div x-data="{ open: false }">
                <button @click="open = !open"
                    class="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-blue-500 transition focus:outline-none">
                    <span class="flex items-center">
                        <i class="fas fa-utensils mr-2 w-5"></i> Manage Foods
                    </span>
                    <i class="fas fa-chevron-down text-xs transition-transform duration-200"
                        :class="{'rotate-180': open}"></i>
                </button>
                <div x-show="open" x-collapse class="mt-1 pl-8 space-y-1">
                    <a href="<%= request.getContextPath() %>/foods?action=addForm"
                        class="block px-3 py-2 text-sm rounded-lg hover:bg-blue-600 transition text-blue-100 hover:text-white flex items-center">
                        <i class="fas fa-plus mr-2 text-xs"></i> Add Food
                    </a>
                    <a href="<%= request.getContextPath() %>/foods?action=list"
                        class="block px-3 py-2 text-sm rounded-lg hover:bg-blue-600 transition text-blue-100 hover:text-white flex items-center">
                        <i class="fas fa-list mr-2 text-xs"></i> All Foods
                    </a>
                </div>
            </div>
